...
pub extern "vm_hooks" fn storage_load_bytes32(key u,dest: *u8) void;
...
pub extern "vm_hooks" fn create2(const code u8*, const code_len usize, endowment u8*, salt u8*, contract u8*, revert_data_len usize*) void;

pub extern "vm_hooks" fn delegate_call_contract(const contract u8*,const calldata u8*, const calldata_len usize, const gas u64, return_data_len usize*) u8;
...
pub extern "vm_hooks" fn memory_grow(const pages u16* ) void;

pub extern "vm_hooks" fn msg_sender(const sender u8* ) void;

pub extern "vm_hooks" fn msg_value(const value u8* ) void;

pub extern "vm_hooks" fn native_keccak256(const bytes u8*, len usize, output u8*) void;

pub extern "vm_hooks" fn read_args(const data u8*) void;

pub extern "vm_hooks" fn read_return_data(dest u8*, offset usize, size usize) void;

pub extern "vm_hooks" fn write_result(const data u8*, len usize) void;

pub extern "vm_hooks" fn return_data_size() usize;

pub extern "vm_hooks" fn static_call_contract(const contract u8*, const calldata u8*, const calldata_len usize, const gas u64, return_data_len usize*) u8;

pub extern "vm_hooks" fn tx_gas_price(const gas_price u8*) void;
